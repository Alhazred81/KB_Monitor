...
void handleSupply(AsyncWebServerRequest *request) {
  ...
  html += "<style>"
          ...
          ".bar-low   { background: linear-gradient(90deg, #ea580c, #f97316); }"
  ...
  auto getLevelInfo = [](int level, bool isWater, String& outClass, String& outText) {
    if (level <= 15) {
      outClass = "bar-crit"; outText = "Kritikus (" + String(level) + "%)";
    } else if (level <= 30) {
      outClass = "bar-low"; outText = "Alacsony (" + String(level) + "%)";
    } else if (level >= 90) {
      outClass = isWater ? "bar-water" : "bar-syrup"; outText = "Maximum (" + String(level) + "%)";
    } else {
      outClass = isWater ? "bar-water" : "bar-syrup"; outText = "Normál (" + String(level) + "%)";
    }
  };
  ...
  html += "<div class='supply-card'><div class='supply-header'><div class='supply-title'>💧 Fő Itató (ID: W1A2)</div><div class='supply-status'>🟢 Aktív (3 perce)</div></div>";
  html += "<div class='info-row'><span>Akku: <b>3.9V</b></span> <span>Dőlés: <b>OK</b></span></div>";
  html += "<div class='prog-bg'><div class='prog-bar " + waterClass + "' style='width: " + String(waterLevel) + "%;'>" + waterText + "</div></div>";
  html += "<div class='info-row'><span style='color:var(--txt2); font-size:12px;'>Szenzor táv.: 22 cm</span> <span style='font-weight:bold;'>Kb. 45 Liter</span></div></div>";
  ...
  html += "<div class='supply-card'><div class='supply-header'><div class='supply-title'>🍯 1. Sor Etető (ID: S3B4)</div><div class='supply-status'>🟢 Aktív (1 perce)</div></div>";
  ...
  html += "<div class='prog-bg'><div class='prog-bar " + syrupClass + "' style='width: " + String(barWidth) + "%;'>" + syrupText + "</div></div>";
  ...
  html += "<button class='btn-edit' onclick=\"let m=prompt('Mit töltöttél az adagolóba?','" + currentMix + "'); if(m){ alert('Mentve: '+m); location.reload(); }\">📝 Tartalom módosítása</button></div>";
